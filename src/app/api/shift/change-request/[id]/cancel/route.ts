...
export async function POST(
  ...
  try {
    ...
    if (oldStatus === RecordStatus.PENDING) {
      await prisma.shiftChangeRequest.delete({ where: { id } });
    ...
    if (oldStatus === RecordStatus.APPROVED) {
      ...
      // Notify approvers: manager or admins
      ...
      const notify = async (recipientId: string, message: string) => {
        ...
        await createNotification(
          recipientId,
          "Shift change cancellation requested",
          message,
          `/shift`
        );
      };
      ...
      const message = `${record.employee.name} yêu cầu huỷ đổi ca hiệu lực từ ${dateStr}`;
    ...
    return Response.json(
      { error: `Cannot cancel request in ${oldStatus} status` },
      { status: 400 }
    );
  } catch (error: unknown) {
