...
export async function POST(
  ...
  try {
    ...
    if (oldStatus === RecordStatus.PENDING) {
      await prisma.flexTimeRecord.delete({ where: { id } });
    ...
    if (oldStatus === RecordStatus.APPROVED) {
      ...
      const notify = async (recipientId: string, message: string) => {
        ...
        await createNotification(
          recipientId,
          "Flex time cancellation requested",
          message,
          `/flex-time`
        );
      };
      ...
      const message = `${record.employee.name} yêu cầu huỷ ${typeLabel} ngày ${dateStr} (${record.minutes} phút)`;
    ...
    return Response.json(
      { error: `Cannot cancel record in ${oldStatus} status` },
      { status: 400 }
    );
  } catch (error: unknown) {
